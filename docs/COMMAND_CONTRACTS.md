...
| `dashboard` | yes | project dashboard and next-actions view | invalid limit or overwrite refusal | read-only unless `--out`; `--no-audit-log` keeps reports deterministic |
