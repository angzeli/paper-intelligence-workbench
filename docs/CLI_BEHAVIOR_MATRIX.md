...
| Registry | `paperwb validate-registry data/registries/example_papers.csv` | Read-only | Not applicable | Path-based | Registry tests |
...
| Claims | `paperwb claims data/notes --output scratch/claims.csv` | Writes explicit output | Existing behavior covered by tests | Yes | Notes/CLI tests |
